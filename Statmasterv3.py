...
def get_manual_stats():
    ...
    while True:
        try:
            entry = input("enter 6 scores seperated by spaces e.g., 15 14 13 12 10 8")
            scores = [int(s) for s in entry.split()]
            if len(scores) == 6:
                return sorted(scores, reverse=True), "N/A (Manual)"
...
if __name__ == "__main__":
    ...
    print("1. Roll 4d6 (Drop Lowest + 7th Roll Mulligan)")
    print("2. Manual Input (Standard Array, Point Buy, or Custom)")
    gen_choice = input("Choice (1 or 2): ")

    # Corrected logic to honor the user's choice
    if gen_choice == "1":
        base_rolls, dropped = generate_full_stat_block()
    else:
        base_rolls, dropped = get_manual_stats()
    
    # REMOVED: base_rolls, dropped = generate_full_stat_block() 
    

    print(f"\nYour Base Rolls: {base_rolls} (Mulligan/Info: {dropped})")

    # Selection for background bonuses
